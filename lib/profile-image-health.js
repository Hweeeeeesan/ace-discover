...
export const IMAGE_SOURCE_HEALTH_STATES = Object.freeze({
  ...
  FAILED: 'failed',
  NO_SOURCE: 'no_source',
  INTENTIONALLY_CLEARED: 'intentionally_cleared',
});
...
function displayedImageSource(profile, galleryCount) {
  if (profile?.imageClearedByAdmin === true) {
    return { key: 'placeholder', label: 'Placeholder · intentionally cleared' };
  }
...
export function classifyProfileImageHealth(profile = {}, inspection = null, inspectionError = null) {
  ...
  if (profile.imageClearedByAdmin === true) {
    return result({
      profile,
      source,
      state: IMAGE_SOURCE_HEALTH_STATES.INTENTIONALLY_CLEARED,
      summary: 'The profile image was intentionally removed by Admin.',
      detail: 'Legacy and Drive fallbacks are suppressed. Upload a new image or explicitly replace/import the gallery to restore it.',
      focalMessage: 'No profile image. Image intentionally removed by Admin.',
      canPreviewImport: false,
      canImport: false,
      acceptableForReplacement: false,
    });
...
export async function inspectProfileImageHealth({
  ...
  if (
    initial.state === IMAGE_SOURCE_HEALTH_STATES.READY
    || initial.state === IMAGE_SOURCE_HEALTH_STATES.INTENTIONALLY_CLEARED
    || !classifyProfileImageSource(profile).eligible
  ) {
    return initial;
  }
  if (driveAuthError) {
