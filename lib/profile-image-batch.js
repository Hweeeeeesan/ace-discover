...
export function profileNeedsImageImport(profile = {}) {
  if (profile.imageClearedByAdmin === true) return false;
...
export function imageImportStatus(dataset, profiles = []) {
  ...
  return {
    ...
    profilesSkippedIntentionallyCleared: profiles.filter((profile) => profile.imageClearedByAdmin === true).length,
    profilesSkippedExistingGallery: profiles.filter((profile) => (
      profile.imageClearedByAdmin !== true
      && (
...
export function presentImageImportResult(dataset, migration, { dryRun }) {
  ...
  return {
    ...
    summary: {
      ...
      profilesSkippedIntentionallyCleared: migration.summary.intentionallyCleared,
