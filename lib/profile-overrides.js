...
import { isMajorGroup, normalizeMajorGroup, resolveMajorGroup } from './import/major-group.js';
...
export function resolveEffectivePublicProfile(importedPublicData = {}, publicOverrides = {}) {
  ...
  for (const field of PUBLIC_PROFILE_EDITABLE_FIELDS) {
    if (field === 'majorGroup') {
      if (Object.prototype.hasOwnProperty.call(imported, 'major') || Object.prototype.hasOwnProperty.call(overrides, field)) {
        effective[field] = resolveMajorGroup(imported.major, overrides[field]);
      }
    }
    else if (Object.prototype.hasOwnProperty.call(overrides, field)) effective[field] = overrides[field];
  }
  return effective;
}
...
export function validatePublicOverrides(value) {
  ...
  for (const [field, rawValue] of Object.entries(value)) {
    if (field === 'majorGroup') {
      if (typeof rawValue !== 'string' || !isMajorGroup(rawValue)) throw new Error('Major category must be a canonical major group.');
      normalized.majorGroup = rawValue;
      continue;
    }
    if (field === 'vibes') {
...
export function buildPublicOverridePatch(importedPublicData = {}, currentOverrides = {}, requestedValues = {}) {
  ...
  for (const [field, value] of Object.entries(normalized)) {
    const automaticValue = field === 'majorGroup'
      ? normalizeMajorGroup(imported.major)
      : imported[field];
    if (JSON.stringify(value) !== JSON.stringify(automaticValue)) next[field] = value;
  }
...
export function buildSinglePublicOverridePatch(
  ...
  const automaticValue = normalizedField === 'majorGroup'
    ? normalizeMajorGroup(importedPublicData?.major)
    : importedPublicData?.[normalizedField];
  if (JSON.stringify(normalized[normalizedField]) === JSON.stringify(automaticValue)) {
