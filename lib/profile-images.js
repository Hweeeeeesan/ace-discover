...
export function isValidProfileImageAssetPath(value) {
  return isValidStorageImagePath(value) || isValidDiscoveryStorageImagePath(value);
}
...
export function resolveProfileImageSourcesForImage(image = {}, options = {}) {
  ...
  const candidates = Array.from(new Set([storageUrl, PROFILE_IMAGE_PLACEHOLDER].filter(Boolean)));
  return {
    src: candidates[0] || PROFILE_IMAGE_PLACEHOLDER,
    candidates,
  };
