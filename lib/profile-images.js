...
const DISCOVERY_IMAGE_FILENAME = /^([0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})-discovery\.webp$/;
const PROFILE_DETAIL_IMAGE_FILENAME = /^([0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})-profile\.webp$/;
...
export function buildDiscoveryDerivativeStoragePath(datasetSlug, profileId, imageId) {
  const dataset = String(datasetSlug || '');
  const profile = String(profileId || '');
  const id = String(imageId || '').toLowerCase();
  if (!PATH_SEGMENT.test(dataset)) throw new Error('Invalid dataset slug for a profile image path.');
  if (!PATH_SEGMENT.test(profile)) throw new Error('Invalid profile ID for a profile image path.');
  if (!IMAGE_ID.test(id)) throw new Error('Invalid image ID for a profile image path.');
  return `${dataset}/${profile}/derived/${id}-discovery.webp`;
}

export function buildProfileDetailDerivativeStoragePath(datasetSlug, profileId, imageId) {
  const dataset = String(datasetSlug || '');
  const profile = String(profileId || '');
  const id = String(imageId || '').toLowerCase();
  if (!PATH_SEGMENT.test(dataset)) throw new Error('Invalid dataset slug for a profile image path.');
  if (!PATH_SEGMENT.test(profile)) throw new Error('Invalid profile ID for a profile image path.');
  if (!IMAGE_ID.test(id)) throw new Error('Invalid image ID for a profile image path.');
  return `${dataset}/${profile}/derived/${id}-profile.webp`;
}
...
export function isValidDiscoveryStorageImagePath(value) {
  ...
  return parts.length === 4
    ...
    && PATH_SEGMENT.test(parts[1])
    && parts[2] === 'derived'
    && DISCOVERY_IMAGE_FILENAME.test(parts[3]);
}

export function isValidProfileDetailStorageImagePath(value) {
  if (typeof value !== 'string' || value.length > 280) return false;
  const parts = value.split('/');
  return parts.length === 4
    && PATH_SEGMENT.test(parts[0])
    && PATH_SEGMENT.test(parts[1])
    && parts[2] === 'derived'
    && PROFILE_DETAIL_IMAGE_FILENAME.test(parts[3]);
}

export function isValidProfileImageAssetPath(value) {
  return isValidStorageImagePath(value)
    || isValidDiscoveryStorageImagePath(value)
    || isValidProfileDetailStorageImagePath(value);
}
...
export function profileImagePresentationStyle({
  focalX,
  focalY,
  displayMode,
} = {}) {
  const normalizedX = normalizeFocalCoordinate(focalX, DEFAULT_FOCAL_X);
  const normalizedY = normalizeFocalCoordinate(focalY, DEFAULT_FOCAL_Y);
  return {
    objectFit: normalizeDisplayMode(displayMode) === 'portrait' ? 'contain' : 'cover',
    objectPosition: `${normalizedX}% ${normalizedY}%`,
  };
}

export function getProfileImages(profile = {}) {
  if (profile.imageClearedByAdmin === true) return [];
  ...
  return profile.profileImages
    .map((image) => ({
      ...
      ...(typeof image?.profileStorageImagePath === 'string' && image.profileStorageImagePath
        ? { profileStorageImagePath: image.profileStorageImagePath }
        : {}),
    ...
    .filter((image) => {
      if (
        (!isValidStorageImagePath(image.storageImagePath)
          && !isValidProfileDetailStorageImagePath(image.storageImagePath))
        || seenPaths.has(image.storageImagePath)
      ) return false;
...
export function storageImagePublicUrl({ supabaseUrl, bucket, storagePath }) {
  if (!isValidProfileImageAssetPath(storagePath)) return '';
...
export function resolveProfileImageSources(profile = {}, options = {}) {
  if (profile.imageClearedByAdmin === true) {
    return { src: PROFILE_IMAGE_PLACEHOLDER, candidates: [PROFILE_IMAGE_PLACEHOLDER] };
  }
  const primaryImage = getPrimaryProfileImage(profile);
  const resolvedPrimary = primaryImage
    ? resolveProfileImageSourcesForImage(primaryImage, options)
    : { src: '', candidates: [] };
  const legacyCandidates = Array.isArray(profile.imageCandidates) ? profile.imageCandidates : [];
  const candidates = Array.from(new Set([
    resolvedPrimary.src,
    ...resolvedPrimary.candidates.filter((candidate) => candidate !== PROFILE_IMAGE_PLACEHOLDER),
    profile.image,
    ...legacyCandidates,
...
/**
 * Resolve one relational image row without falling back to the profile's
 * primary image. This is used by Admin cards and the public detail gallery so
 * every slide remains tied to its own Storage object.
 */
export function resolveProfileImageSourcesForImage(image = {}, options = {}) {
  const preferredPath = isValidProfileDetailStorageImagePath(image.profileStorageImagePath)
    ? image.profileStorageImagePath
  ...
  const canonicalUrl = preferredPath !== image.storageImagePath
    ? storageImagePublicUrl({
      supabaseUrl: options.supabaseUrl,
      bucket: options.bucket || DEFAULT_PROFILE_IMAGE_BUCKET,
      storagePath: image.storageImagePath,
    })
    : '';
  const candidates = Array.from(new Set([
    storageUrl,
...
export function withResolvedProfileImage(profile = {}, options = {}) {
  ...
  return {
    ...profile,
    profileImages: getProfileImages(profile).map((image) => ({
      ...image,
      ...resolveProfileImageSourcesForImage(image, options),
    })),
