...
function fall2026AceTraitSlideColumns(headers, idealHangoutByRole) {
  const slideColumns = matchingHeaderColumns(headers, ['subtle ace trait', 'slide']);
  if (slideColumns.length === 0) return { Little: null, Family: null, Big: null };
  if (slideColumns.length > 2) {
    throw new Error('BIGS schema mismatch: Fall 2026 Little and Big ACE Trait slide headers could not be resolved safely.');
  }

  const hobbyStarts = matchingHeaderColumns(headers, ['list your', 'favorite hobbies/activities']);
  const bigIdealHangout = idealHangoutByRole?.Big || null;
  if (hobbyStarts.length === 2 && bigIdealHangout) {
    const bigCandidates = slideColumns.filter((column) => (
      columnIndex(column) > columnIndex(hobbyStarts[1])
      && columnIndex(column) < columnIndex(bigIdealHangout)
    ));
    const littleCandidates = slideColumns.filter((column) => columnIndex(column) > columnIndex(bigIdealHangout));
    if (
      bigCandidates.length <= 1
      && littleCandidates.length <= 1
      && bigCandidates.length + littleCandidates.length === slideColumns.length
    ) {
      return {
        Little: littleCandidates[0] || null,
        Family: null,
        Big: bigCandidates[0] || null,
      };
    }
  }

  // Legacy exports may predate one of the two optional questions. Exact known
  // columns are accepted only when their header still identifies the field.
  const knownColumns = new Set(slideColumns);
  if ([...knownColumns].every((column) => ['CZ', 'DB'].includes(column))) {
    return {
      Little: knownColumns.has('DB') ? 'DB' : null,
      Family: null,
      Big: knownColumns.has('CZ') ? 'CZ' : null,
    };
  }
  throw new Error('BIGS schema mismatch: Fall 2026 Little and Big ACE Trait slide headers could not be resolved safely.');
}
...
function selectSheetConfig(sheetName, headers) {
  ...
  if (sheetName === 'LITTLES' && String(headers.BC || '').toLowerCase().includes('instagram')) {
    ...
    const aceTraitSlideByRole = fall2026AceTraitSlideColumns(headers, idealHangoutByRole);
    Object.assign(config, {
      ...
      hotTake: ['Z', 'CH'], idealHangoutByRole, aceTraitSlideByRole,
      instagram: 'K', image: ['AQ', 'CX'],
...
export function detectPublicPiiKinds(value) {
  const text = cleanText(value);
  const kinds = [];
  if (new RegExp(EMAIL_PATTERN, 'i').test(text)) kinds.push('email');
  if (new RegExp(PHONE_PATTERN).test(text) || new RegExp(SCIENTIFIC_PHONE_PATTERN, 'i').test(text)) kinds.push('phone');
  if (new RegExp(PRIVATE_KEY_PATTERN, 'i').test(text) || new RegExp(SENSITIVE_PARAMETER_PATTERN, 'i').test(text)) kinds.push('credential');
  return kinds;
}
...
function buildProfiles(tabTitle, values) {
  ...
  values.slice(1).forEach((valuesRow, index) => {
    ...
    if (config.aceTraitSlideByRole) {
      aceTraitSlideUrl = normalizePublicHttpUrl(first(row, config.aceTraitSlideByRole[role]));
    }
