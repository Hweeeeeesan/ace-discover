...
function fall2026PassionColumns(headers) {
  ...
  const passionColumns = matchingHeaderColumns(headers, ['passionate', 'talk about', 'hours']);
  if (hobbyStarts.length < 2) {
    throw new Error('BIGS schema mismatch: Fall 2026 Little and Big passion headers could not be resolved safely.');
  }
  const littleCandidates = passionColumns.filter((column) => (
    columnIndex(column) > columnIndex(hobbyStarts[0])
    && columnIndex(column) < columnIndex(hobbyStarts[1])
  ));
  const bigCandidates = passionColumns.filter((column) => (
    columnIndex(column) > columnIndex(hobbyStarts[1])
  ));
  if (littleCandidates.length !== 1 || bigCandidates.length !== 1) {
    throw new Error('BIGS schema mismatch: Fall 2026 Little and Big passion headers could not be resolved safely.');
  }
  return {
...
export function scoreVibeEvidence(fields = {}) {
  ...
  for (const vibeConfig of VIBE_SCORING.vibes) {
    ...
    for (const [field, fieldWeight] of Object.entries(VIBE_SCORING.fieldWeights)) {
      const text = cleanText(fields?.[field]).toLowerCase();
      if (!text) continue;
      const accepted = [];
