...
const VIBE_RULES = {
  Foodie: [/\bfoodie\b/i, /\btrying (?:new )?food\b/i, /\beating out\b/i, /\bcook(?:ing)?\b/i, /\bbak(?:e|ing)\b/i, /\brestaurant/i],
  Outdoors: [/\bhik(?:e|ing)\b/i, /\bcamp(?:ing)?\b/i, /\bnature\b/i, /\bbeach\b/i, /\btrail\b/i, /\bbackpack(?:ing)?\b/i],
  Gaming: [/\bvideo games?\b/i, /\bgaming\b/i, /\bvalorant\b/i, /\bleague of legends\b/i, /\bfortnite\b/i, /\broblox\b/i, /\btft\b/i],
  Music: [/\bmusic\b/i, /\bconcerts?\b/i, /\bsing(?:ing)?\b/i, /\bguitar\b/i, /\bpiano\b/i, /\bdj(?:ing)?\b/i, /\braves?\b/i],
  Creative: [/\bdraw(?:ing)?\b/i, /\bpaint(?:ing)?\b/i, /\bcraft(?:s|ing)?\b/i, /\bcreative writing\b/i, /\bscrapbook(?:ing)?\b/i, /\bdesign(?:ing)?\b/i],
  Fitness: [/\bgym\b/i, /\bwork(?:ing)? out\b/i, /\bweightlift(?:ing)?\b/i, /\bfitness\b/i, /\bbodybuilding\b/i, /\brunning\b/i],
  Sports: [/\bvolleyball\b/i, /\bbasketball\b/i, /\bsoccer\b/i, /\bbadminton\b/i, /\bpickleball\b/i, /\bfootball\b/i, /\btennis\b/i, /\bsports?\b/i],
  Travel: [/\btravel(?:ing|ling)?\b/i, /\broad trips?\b/i, /\bexplor(?:e|ing) new (?:cities|places)\b/i, /\bvisit(?:ing)? (?:new )?(?:countries|places)\b/i],
  'Movies & TV': [/\bmovies?\b/i, /\bfilms?\b/i, /\btv shows?\b/i, /\bk-?dramas?\b/i, /\bsitcoms?\b/i, /\bnetflix\b/i],
  Anime: [/\banime\b/i, /\bmanga\b/i, /\bmanhwa\b/i, /\bjujutsu kaisen\b/i, /\bdemon slayer\b/i],
  Nightlife: [/\bnightlife\b/i, /\bclub(?:bing)?\b/i, /\bbars?\b/i, /\braves?\b/i, /\bpart(?:y|ies|ying)\b/i],
  'Coffee & Cafes': [/\bcaf[eé]s?\b/i, /\bcafe hopping\b/i, /\bcoffee shops?\b/i, /\bmatcha\b/i, /\bboba\b/i],
  Studying: [/\bstudy(?:ing)?\b/i, /\bstudy sessions?\b/i, /\blibrary\b/i, /\bacademic(?:s)?\b/i],
  Fashion: [/\bfashion\b/i, /\bthrift(?:ing)?\b/i, /\bshopping\b/i, /\bstreetwear\b/i, /\bclothes?\b/i, /\boutfits?\b/i],
  Photography: [/\bphotograph(?:y|er|ing)?\b/i, /\btaking photos?\b/i, /\bdigicam\b/i, /\bcamera\b/i],
  Volunteering: [/\bvolunteer(?:ing)?\b/i, /\bcommunity service\b/i, /\bgiving back\b/i, /\bnonprofit\b/i, /\bcharity\b/i],
};
...
function selectSheetConfig(sheetName, headers) {
  ...
  if (sheetName === 'LITTLES' && String(headers.BC || '').toLowerCase().includes('instagram')) {
    ...
    Object.assign(config, {
      ...
      movies: ['V', 'CB'], passion: ['AH', 'CC'], tagline: ['X', 'CD'],
      perfectDay: ['Y', 'CE'], uniqueThings: ['W', 'CF'], bucketList: ['AB', 'CG'],
      hotTake: ['Z', 'CH'], idealHangout: 'AA', instagram: 'K', image: ['AQ', 'CX'],
...
function interestTags(hobbies, role) {
  const text = redactPii(hobbies);
  const candidates = [];
  const keywordMap = [
  ...
  const lower = text.toLowerCase();
  for (const [label, keyword] of keywordMap) {
    if (lower.includes(keyword) && !candidates.includes(label)) candidates.push(label);
    if (candidates.length >= 3) break;
  }
  if (!candidates.length) {
    for (let part of text.split(/[,;•\n]|\s+-\s+/)) {
      part = cleanText(part.replace(/^[-–—\d.)\s]+/, ''));
      part = part.split(/\bbecause\b|\bI like\b|\bI love\b/i)[0].replace(/^[ .:-]+|[ .:-]+$/g, '');
      if (part.length >= 2 && part.length <= 26 && !['n/a', 'none'].includes(part.toLowerCase())) candidates.push(part.slice(0, 26));
      if (candidates.length >= 3) break;
    }
  }
  return (candidates.length ? candidates : [role]).slice(0, 3);
}

function inferVibes(...values) {
  const text = cleanText(values.map((value) => String(value || '')).join(' ')).toLowerCase();
  return VIBE_ORDER.filter((vibe) => VIBE_RULES[vibe].some((pattern) => pattern.test(text)));
}
...
function buildProfiles(tabTitle, values) {
  ...
  values.slice(1).forEach((valuesRow, index) => {
    ...
    let interests = interestTags(hobbies, role, program);
    if (config.f26 && role === 'Little') interests = [role, ...interests.filter((item) => item !== role).slice(0, 2)];
    ...
    profiles.push({
      ...
      interests,
      vibes: inferVibes(hobbies, music, movies, perfectDay, story),
