...
function fallbackDataset() {
  return {
    ...FALL_2025_DATASET,
    profiles: resolveProfileImages(fallProfiles.map(discoveryProfile)),
    health: importHealth,
  };
}
...
function normalizePayload(payload) {
  ...
  return {
    ...dataset,
    // The public bulk RPC already returns the effective merged profile. Keep
    // this normalization path identical for local fallback and Supabase data.
    profiles: resolveProfileImages(payload.profiles.map(discoveryProfile)),
...
export async function getPublishedProfile(datasetSlug, profileId) {
  ...
  if (active.source === 'local-fallback') {
    ...
    return profile ? { dataset: active, profile: resolveProfileImage(profile) } : null;
  }
  ...
  return {
    ...
    profile: resolveProfileImage(resolveEffectivePublicProfile(data.profile)),
  };
}
