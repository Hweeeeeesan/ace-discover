...
export async function getAdminDatasetProfile(datasetId, profileId) {
  ...
  const { data: row, error: profileError } = await supabase
    .from('dataset_profiles')
    .select('public_data,public_overrides,public_overrides_updated_at,public_overrides_updated_by,storage_image_path')
    .eq('dataset_id', datasetId)
  ...
  const effectivePublicData = resolveEffectivePublicProfile(importedPublicData, publicOverrides);
  const resolvedProfile = resolveProfileImage({ ...row.public_data, profileImages });
  Object.assign(resolvedProfile, effectivePublicData);
