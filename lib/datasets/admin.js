...
export async function getAdminDatasetProfile(datasetId, profileId) {
  ...
  const { data: row, error: profileError } = await supabase
    .from('dataset_profiles')
    .select('public_data,public_overrides,public_overrides_updated_at,public_overrides_updated_by,drive_file_id,drive_folder_id,image_kind,image_issue,storage_image_path,image_cleared_by_admin')
    .eq('dataset_id', datasetId)
