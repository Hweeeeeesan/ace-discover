...
import { isValidStorageImagePath } from '../profile-images';
...
export async function getAdminDatasetProfile(datasetId, profileId) {
  ...
  const profileImagesWithSources = profileImages.map((image) => {
    const resolved = resolveProfileImage({ ...row.public_data, profileImages: [image] });
    return {
      ...image,
      src: resolved.image,
      candidates: resolved.imageCandidates,
    };
  });
