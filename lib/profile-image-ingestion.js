...
export async function ingestProfileImages({
  ...
  for (const candidate of gallery.candidates) {
    ...
    const profileStoragePath = buildProfileDetailDerivativeStoragePath(datasetSlug, profile.id, imageId);
    ...
    stagedImages.push({
      ...
      profileBytes: assets.profile.bytes,
  ...
  if (!dryRun && (!requireAllSupported || allSupportedValidated)) {
    for (const staged of stagedImages) {
      ...
      try {
        ...
        await upload({
          storagePath: staged.profileStoragePath,
          bytes: staged.profileBytes,
          contentType: staged.profileMimeType,
        });
        uploadedPaths.push(staged.profileStoragePath);
        await upload({
      ...
      const { bytes, profileBytes, discoveryBytes, ...image } = staged;
      images.push(image);
    }
  } else if (dryRun) {
    images.push(...stagedImages.map(({ bytes, profileBytes, discoveryBytes, ...image }) => image));
  }
