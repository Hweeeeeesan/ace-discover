...
export async function generateProfileDetailDerivative(bytes, options = {}) {
  return renderWebp(bytes, {
    longEdge: positiveInteger(options.longEdge, PROFILE_DETAIL_LONG_EDGE),
    quality: webpQuality(options.quality, PROFILE_DETAIL_WEBP_QUALITY),
    maxInputPixels: options.maxInputPixels,
  });
}

/**
 * Keep the validated/orientation-normalized source as the canonical recovery
 * object and build the two public delivery derivatives from those pixels.
 */
export async function generateProfileImageAssetsFromNormalized(normalized, options = {}) {
  ...
  const profile = await generateProfileDetailDerivative(normalized.bytes, {
    longEdge: positiveInteger(
      options.profileLongEdge ?? options.canonicalLongEdge,
      PROFILE_DETAIL_LONG_EDGE,
    ),
    quality: webpQuality(
      options.profileQuality ?? options.canonicalQuality,
      PROFILE_DETAIL_WEBP_QUALITY,
    ),
    maxInputPixels: options.maxInputPixels,
  });
  const discovery = await generateDiscoveryDerivative(profile.bytes, {
...
export async function deriveMissingProfileDetailImages({
  images,
  dryRun = true,
  concurrency = 2,
  download,
  ...
  remove,
  buildDerivativePath,
  derive = generateProfileDetailDerivative,
  onRow = () => {},
}) {
  if (!Array.isArray(images)) throw new Error('Profile images must be an array.');
  if (typeof download !== 'function') throw new Error('A canonical image downloader is required.');
  ...
  const selected = images.filter((image) => !image.profileStoragePath);
  const results = await mapWithConcurrency(selected, concurrency, async (image) => {
    const base = {
      imageId: image.id,
      profileId: image.profileId,
    ...
    try {
      ...
      const row = {
        ...
        currentWidth: derivative.sourceWidth,
        currentHeight: derivative.sourceHeight,
        profileStoragePath: derivativePath,
        profileWidth: derivative.width,
        profileHeight: derivative.height,
        profileMimeType: derivative.contentType,
        profileByteLength: derivative.byteLength,
      };
      if (!dryRun) {
        await upload({ storagePath: derivativePath, bytes: derivative.bytes, contentType: derivative.contentType });
        try {
          await updateMetadata({ image, derivativePath, derivative });
        } catch (error) {
          if (!error?.preserveStagedObject) {
  ...
  results.forEach(onRow);
  const successful = results.filter((row) => ['dry_run_derived', 'derived'].includes(row.status));
  const totalCurrentBytes = successful.reduce((total, row) => total + row.currentBytes, 0);
  const totalDerivativeBytes = successful.reduce((total, row) => total + row.profileByteLength, 0);
  const currentPrimaryBytes = successful.filter((row) => row.isPrimary)
    .reduce((total, row) => total + row.currentBytes, 0);
  const derivativePrimaryBytes = successful.filter((row) => row.isPrimary)
    .reduce((total, row) => total + row.profileByteLength, 0);
  return {
    rows: results,
    summary: {
      imagesScanned: images.length,
      alreadyDerived: images.length - selected.length,
      imagesNeedingDerivative: selected.length,
      derived: successful.length,
      failures: results.filter((row) => row.status === 'failed').length,
      skippedIntentionallyCleared: results.filter((row) => row.status === 'skipped_intentionally_cleared').length,
      ...
      reductionPercent: totalCurrentBytes > 0
        ? Math.round((1 - totalDerivativeBytes / totalCurrentBytes) * 1000) / 10
        : 0,
      currentPrimaryBytes,
      derivativePrimaryBytes,
      primaryReductionPercent: currentPrimaryBytes > 0
