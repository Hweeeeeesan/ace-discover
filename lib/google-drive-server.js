...
const MAX_IMAGE_BYTES = MAX_PROFILE_IMAGE_INPUT_BYTES;
const SUPPORTED_PROFILE_IMAGE_TYPES = new Set(SUPPORTED_PROFILE_IMAGE_MIME_TYPES);
...
export class DriveImageSourceError extends Error {
  constructor(code, message, details = {}) {
    super(message);
    this.name = 'DriveImageSourceError';
    this.code = code;
...
export async function listImageFilesInFolder(folderId, auth) {
  ...
  return children.filter((file) => SUPPORTED_PROFILE_IMAGE_TYPES.has(
...
export async function fetchDriveImage(fileId, auth) {
  ...
  const sourceMime = String(metadata?.mimeType || '').toLowerCase();
  if (sourceMime && !SUPPORTED_PROFILE_IMAGE_TYPES.has(sourceMime)) {
    throw new DriveImageSourceError(
      'unsupported_file_type',
      sourceMime === 'application/pdf'
        ? 'The Google Drive source is a PDF, not an image.'
        : `The Google Drive source type ${sourceMime} is not supported as a profile image.`,
      { mimeType: sourceMime },
    );
  }
