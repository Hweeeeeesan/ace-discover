...
export const DISCOVERY_MODES = ['All', 'Unseen', 'Vibes'];
...
export function filterAndOrderProfiles(profiles, values = {}) {
  ...
  const mode = DISCOVERY_MODES.includes(values.mode) ? values.mode : 'All';
  const seenIds = new Set(Array.isArray(values.seenIds) ? values.seenIds : []);
  const selectedVibes = Array.isArray(values.selectedVibes)
    ? values.selectedVibes.filter((vibe) => VIBE_OPTIONS.includes(vibe))
    : [];
  const filtered = profiles.filter((profile) => {
    if (mode === 'Unseen' && seenIds.has(profile.id)) return false;
    if (
      selectedVibes.length > 0
      && !selectedVibes.some((vibe) => profile.vibes?.includes(vibe))
    ) return false;
    return matchesFilters(profile, role, filters);
  });
  const normalizedQuery = normalizeText(query);
