...
import { MAJOR_GROUP_ORDER, resolveMajorGroup } from './import/major-group.js';
...
export const MAJOR_GROUP_OPTIONS = MAJOR_GROUP_ORDER;
...
export function getDiscoveryOptions(profiles) {
  ...
  profiles.forEach((profile) => {
    ...
    const effectiveMajorGroup = resolveMajorGroup(profile.major, profile.majorGroup);
...
function publicFieldValue(profile, field) {
  ...
  if (field === 'majorGroup') return resolveMajorGroup(profile.major, profile.majorGroup);
...
function matchesFilters(profile, role, filters) {
  ...
  if (filters.majorGroups.length > 0 && !filters.majorGroups.includes(optionKey(resolveMajorGroup(profile.major, profile.majorGroup)))) {
    return false;
  }
