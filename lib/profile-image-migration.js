...
export function buildGalleryReplacementPlan(existingImages, replacementImages) {
  ...
  return {
    ...
    oldStoragePaths: existing.flatMap((image) => (
...
function stagedStoragePaths(images) {
      ...
      if (replaceExisting) {
        ...
        if (!replacementReady) {
          const cleanupFailures = dryRun
            ? []
            : await cleanupStoragePaths(stagedStoragePaths(ingested.images), removeStorage);
        ...
        try {
          ...
          const cleanupFailures = error?.preserveStagedObjects
            ...
            : await cleanupStoragePaths(
              stagedStoragePaths(ingested.images),
              removeStorage,
            );
      ...
      if (!dryRun && typeof createGallery === 'function') {
        ...
        for (const image of ingested.images || []) {
          try {
            ...
            let cleanupDetail = '';
            try {
              await cleanupStoragePaths(stagedStoragePaths([image]), removeStorage).then((failures) => {
